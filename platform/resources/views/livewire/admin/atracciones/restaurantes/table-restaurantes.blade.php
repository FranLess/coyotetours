<div>
    ...
    <div class="table-responsive m-3 bg-dark">
        ...
        align-middle">
            ...
            <tbody class="table-group-divider">
                @foreach ($restaurantes as $hotel)
                    <tr class="table-light">
                        <td scope="row">{{ $hotel->id }}</td>
                        <td>{{ $hotel->nombre }}</td>
                        <td> <img width="100" src="{{ asset("img/atracciones/restaurantes/$hotel->imagen") }}"
    ...
    <script>
        window.addEventListener('load', () => {
            Livewire.on('borrarAsk', (restaurante) => {
                let ask = confirm('El registro no podra restaurase');
                console.log(restaurante);
                if (ask) Livewire.emit('borrarRestaurante', restaurante.restaurante);
            });
