<div>
            ...
            <tbody class="table-group-divider">
                @foreach ($imagenes as $imagen)
                    <tr class="table-light">
                        ...
                        <td>{{ $imagen->nombre }}</td>
                        <td> <img width="100" src="{{asset("img/hoteles/habitaciones/$imagen->imagen")}}" class="img-fluid rounded" alt=""></td>
                        <td>
    ...
    <script>
        window.addEventListener('load', () => {
            Livewire.on('borrarAsk', (hotelHabitacionImagen) => {
                let ask = confirm('El registro no podra restaurase');
                if (ask) Livewire.emit('borrarHotelHabitacionImagen', hotelHabitacionImagen.hotelHabitacionImagen);
            });
            Livewire.on('cola' , ()=>{
                alert('cola');
            });
