<div>
            ...
            <tbody class="table-group-divider">
                @foreach ($hoteles as $hotel)
                    <tr class="table-light">
                        ...
                        <td>{{ $hotel->nombre }}</td>
                        <td> <img width="100" src="{{asset("img/hoteles/$hotel->imagen")}}" class="img-fluid rounded" alt=""></td>
    ...
    <script>
        window.addEventListener('load', () => {
            Livewire.on('borrarAsk', (hotel) => {
                let ask = confirm('El registro no podra restaurase');
                console.log(hotel);
                if (ask) Livewire.emit('borrarHotel', hotel.hotel);
            });
